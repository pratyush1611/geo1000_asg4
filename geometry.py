...
class Point(object):
    ...
    def __str__(self):
        ...
        x_val = self.x
        y_val = self.y
        return( f"POINT ({round(x_val, 6)} {round(y_val, 6)})" )
    ...
    def intersects(self, other):
        ...
        if isinstance(other, Point): #point class
            ...
            return( True if((self.x >= other.ll.x and self.x <= other.ur.x ) and (self.y >= other.ll.y and self.y <= other.ur.y )) else False )
